...
def enrich_ceo_data(df, image):
    ceo_payload = gee_converter(df["lat"].to_numpy(), df["lon"].to_numpy(), df["Land_Cover_Elements"].to_numpy(), df["ObjectId"].to_numpy()).tolist()
    ...
    combined["harmonized_ceo"] = ceo_converter(combined["Land_Cover_Elements"].to_numpy())
    combined["harmonized_wc"] = wc_converter(combined["wc_classification"].to_numpy())
    return combined[(combined["harmonized_ceo"] != "") & (combined["harmonized_wc"] != "")]
    
def generate_confusion_matrix(df):
    cf_matrix = confusion_matrix(df["harmonized_ceo"], df["harmonized_wc"], labels=harmonized_classes, normalize="pred")
...
harmonized_wc_vis = [harmonized_classes_vis[wc_to_harmonized_lookup[classification]] for classification in wc_to_harmonized_lookup.keys()]
...
if plotid == entire_aoi_option:
    ...
    st.session_state["analysis_ssu"] = st.session_state["selected_ssu"][st.session_state["selected_ssu"]["plotid"] == plotid]
...
with ssu_col:
    ...
    st.session_state["analysis_ssu"] = enrich_ceo_data(st.session_state["analysis_ssu"], world_cover)
    cf_fig, agreed, confused = generate_confusion_matrix(st.session_state["analysis_ssu"] )
    ...
    st.metric("Most agreed landcover class", agreed_class, delta=agreed_percent, delta_color="off")
    st.metric("Most confused landcover class", confused_str, delta=confused_percent, delta_color="off")
    ...
    Map.addLayer(world_cover, vis_params = {"min" : 10, "max" : 100, "palette": harmonized_wc_vis}, name="World Cover")
    if plotid != entire_aoi_option:
        layer_group = LayerGroup(name="Grid")
        x = []
        y = []
        grid = folium.FeatureGroup(name="100m Grid")
        for _, data in st.session_state["analysis_ssu"][st.session_state["analysis_ssu"]["plotid"] == plotid].iterrows():
...
with st.sidebar:
    ...
    st.download_button(label="Download Primary Sampling Unit CSV", data = selected_psu_data, file_name=f"CEO PSU-{aoi}.csv", mime='text/csv')
    st.download_button(label="Download Secondary Sampling Unit CSV", data = selected_ssu_data, file_name=f"CEO SSU-{aoi}.csv", mime='text/csv')
